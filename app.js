...
(function () {
    ...
    var allTimeCases = void 0;
    var allTimeDiffs = void 0;
    ...
    var transform = function transform(data) {
        ...
        countyKeys.forEach(function (ck) {
            ...
            sortData[ck] = {
                key: ck,
                label: counties[ck],
                cases: [].concat(_toConsumableArray(casesData[ck])),
                caseDiffs: caseDiffs,
                newCases: diff,
                totalCases: last[1]
            };
    ...
    var createAllCasesDOM = function createAllCasesDOM(cases) {
        ...
        var data = {
            series: [[].concat(_toConsumableArray(cases))]
        };
    ...
    var createDOM = function createDOM(sortData) {
        ...
        var articles = sortData.map(function (sd) {
            ...
            var article = document.querySelector("article[data-label=\"" + sd.label + "\"]");
            var header = void 0;
            if (!article) {
            ...
            var data = {
                series: [[].concat(_toConsumableArray(sd.caseDiffs))]
            };
    ...
    var use = function use(data) {
        ...
        var newCases = Object.keys(transformedData).reduce(function (acc, current) {
        ...
        allTimeCases = sortedData.reduce(function (acc, current) {
            if (acc.length === 0) {
                return [].concat(_toConsumableArray(current.cases));
            } else {
        ...
        allTimeDiffs = sortedData.reduce(function (acc, current) {
            if (acc.length === 0) {
                return [].concat(_toConsumableArray(current.caseDiffs));
            } else {
                return acc.map(function (x, i) {
    ...
    var sort = function sort(selection) {
        ...
        sortedArticles.forEach(function (a) {
            ...
            var data = { series: [] };
            data.series[0] = selection === 'newcases' ? [].concat(_toConsumableArray(transformedData[key.toUpperCase()].caseDiffs)) : [].concat(_toConsumableArray(transformedData[key.toUpperCase()].cases));
        ...
        var allData = { series: [] };
        allData.series[0] = selection === 'newcases' ? [].concat(_toConsumableArray(allTimeDiffs)) : [].concat(_toConsumableArray(allTimeCases));
    ...
    var showCounty = function showCounty(e) {
        ...
        if (target.id) {
            ...
            console.log(label);
            var el = document.querySelector("article[data-label=\"" + label + "\"]");
            console.log(el);
            if (el) {
                el.scrollIntoView({
                    behavior: 'smooth'
